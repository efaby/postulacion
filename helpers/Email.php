...
class Email{
	...
	public function sendMail($email_to,$email_subject, $message){
		...
		$headers .= 'From: SISTEMA DE POSTULACIÓN DOCENTE DE LA UESMJ <'.$email_from.">\r\n".
				'Reply-To: '.$email_from."\r\n" .
	...
	public function sendNotificacionPostulacion($name,$postulacion,$email, $etapa, $calificacion, $aprobado){
		...
		$afirmacion = ($aprobado==1)?'Si':'No cumple con las espectativas institucionales y no';
		$message = '<table>
					    <tr>
					      <td>Estimado '.$name.',</td>
					    </tr>
					    <tr>
					      <td> Le informámos que sú postulación a '.$postulacion.', '.$afirmacion.' ha pasado la etapa de '.$etapa.' con una calificación de '.$calificacion.', por favor ingrese al sistema
